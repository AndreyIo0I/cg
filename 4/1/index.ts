...
async function main() {
	...
	function drawScene() {
		...
		const fPosition = [100, 0, 0]

		const cameraPosition = vec3.fromValues(0, 0, 200)
		const up = [0, 1, 0]
		const cameraMatrix = new Float32Array(m4.lookAt(cameraPosition, fPosition, up))
		console.log(
			new Float32Array(m4.lookAt(cameraPosition, fPosition, up)),
			new Float32Array(mat4.lookAt(mat4.create(), new Float32Array(cameraPosition), new Float32Array(fPosition), new Float32Array(up))),
		)
		const viewMatrix = mat4.invert(mat4.create(), new Float32Array(cameraMatrix))
		const viewProjectionMatrix = mat4.multiply(mat4.create(), projectionMatrix, viewMatrix)
		const matrix = mat4.translate(mat4.create(), viewProjectionMatrix, vec3.fromValues(200, 0, 0))
...
const m4 = {
	...
	subtractVectors: function (a: vec3, b: Array<number>) {
	...
	lookAt: function (cameraPosition: vec3, target: Array<number>, up: Array<number>) {
	...
	transformVector: function (m: Array<number>, v: Array<number>) {
		var dst = []
		for (var i = 0; i < 4; ++i) {
			dst[i] = 0.0
			for (var j = 0; j < 4; ++j) {
